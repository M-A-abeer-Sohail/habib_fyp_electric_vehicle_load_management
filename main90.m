...
industkvar = ceil(industkw.*[125 110 130 580 160*3 110 90 120]./ceil([170 130 180 900 185*3 200 180 210])); % In this format if I ever
                                                                                                            % want to change the load on the
homePowerFactor = 0.9; % Can change it later if needed, we have assumed it
...
reskvar = ceil(reskw*tan(acos(homePowerFactor)));

%%
close all;

x=linspace(-3,3,96);
gaussian = @(x, sigma, mu) (1/sigma/sqrt((2*pi))*exp(-((x-mu)/sigma).^2/2));
                % @ Creates a function handle to an anonymous function
skewedgaussian = @(x,sigma,mu,alpha) 2*gaussian(x, sigma, mu).*normcdf(alpha*x);
clear x;

noiseStd = 0.0425; % For adding some randomness to test data
...
tempMults = load('normalMult.mat');
resMult = tempMults.resMult;
industMult = tempMults.industMult;

% This is the latest 06/05/2022
% industMult = skewedgaussian(linspace(-0.5,1.5,npts),0.425,0,9) + noiseStd.*randn(1,npts);
% industMult = industMult./max(industMult)/multb + offsetb;
% industMult = industMult./max(industMult);
% industMult = circshift(industMult, 10);
...
numEVLds = numEVLds + numEVLdsTemp; % Puting it all together
...
evBusNames = [634, 675, 646, 652, 611, 645, 671, 692, 680];

% % Will need to fix these...
% evAtBus = readmatrix('data/evVeh40.csv');
% carAtBus=evAtBus(2:end,1:end-2); % I'll program 634 separately for validation
% bikeAtBus=evAtBus(2:end,end-1:end); % I'll program 634 separately for validation

ev1stSlot = readmatrix('data/1stSlotEV90.csv');
ev2ndSlot = readmatrix('data/2ndSlotEV90.csv');
ev3rdSlot = readmatrix('data/3rdSlotEV90.csv');
ev4thSlot = readmatrix('data/4thSlotEV90.csv');
...
evPowerFactor = 0.9; % Can be changed manually

busNumPh = [3 2 1 1 3 3 3 3 3]; % Used in getEvLoad fcn

% Now we will also have loads on indsutrial buses
str_part2 = ["New Load.EV_634a Bus1=634.1 Phases=1 Conn=Wye  Model=1 kV=0.23 vminpu=0.3 ",...
            ...
            "New Load.EV_645a Bus1=645.1 Phases=1 Conn=Wye  Model=1 kV=6.35 vminpu=0.3 ",...
            "New Load.EV_645b Bus1=645.2 Phases=1 Conn=Wye  Model=1 kV=6.35 vminpu=0.3 ",...
            "New Load.EV_645c Bus1=645.3 Phases=1 Conn=Wye  Model=1 kV=6.35 vminpu=0.3 ",...
...
% [Old version] Generating EV loads for cars
...
% First I'll define load shapes for the 4 time blocks

% plot(x, gaussian(x))
% hold on
% plot(x, skewedgaussian(x, 0))
% plot(x, skewedgaussian(x, -4))
% plot(x, skewedgaussian(x, 1))
% plot(x, skewedgaussian(x, -1))

% evMults = load('evMults.mat');
carpdf = skewedgaussian(linspace(-1,2.5,npts), 0.75, 0, 4.5) + 0.00077 ...
    + 0.015*rand(1,npts);
carpdf = carpdf/max(carpdf);
% bikepdf = evMults.bikepdf;

bikepdf = skewedgaussian(linspace(-2.75,1,npts), 0.9, 0, -3) + 0.000875 ...
    + 0.015*rand(1,npts);% this will multiply
                                                                       % with the
                                                                       % EV load to
                                                                       % simulate gradual rise,
                                                                       % also with some noise
bikepdf = bikepdf/max(bikepdf);
carpdf = circshift(carpdf, 13*nptMult);
bikepdf = circshift(bikepdf,  10*nptMult);

figure;
plot(carpdf); hold on;
plot(bikepdf);
xticks(linspace(1,npts,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});

% coordMult1 = skewedgaussian(linspace(-1,2.5,npts), 0.5, 0, 35) ...
%     + 0.0075*rand(1,npts); % here enter npts
%                         % the skew direction is meaningful, it 
%                         % shows when vehicle owners will abruptly
%                         % will begin charging when they reach their
%                         % workplaces.
% coordMult2 = skewedgaussian(linspace(-1,2.5,npts), 0.5, 0, 10) + 0.0075 ...
%     + 0.0075*rand(1,npts);
                        
% the time blocks are four: 8 am to 1 pm, 2 pm to 6 pm, 6 30pm to 11 30 pm,
% and 12 am to 5 am
% I shifted the signals so that they come close to the mid points of the
                        
% coordMult1 = (coordMult1/max(coordMult1))';
% coordMult2 = (coordMult2/max(coordMult2))';
% coordMultTimeBlock = [circshift(coordMult1, 3*nptMult)...
%                       circshift(coordMult1, 7*nptMult)...
%                       circshift(coordMult2, 13*nptMult)...
%                       circshift(coordMult2, 17*nptMult)];
% coordMultTimeBlock = [circshift(coordMult1, 2*nptMult) zeros(npts,1) zeros(npts,1) zeros(npts,1)];
% plot(coordMult); hold on;

tempMults = load('coordMult.mat');
coordMultTimeBlock = tempMults.coordMultTimeBlock;

figure;
plot(coordMultTimeBlock);
xticks(linspace(1,npts,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
legend('1','2','3','4');
...
% save('normalMult','resMult','industMult');
% save('coordMult','coordMultTimeBlock');
% save('uncoordMult','carpdf','bikepdf');

% clearvars tempMults;
% clearvars carpdf bikepdf;

%%
ViSetting = [[4,1];[4,2];[4,3];[6,1];[6,2];[6,3];[8,1];[9,3];[11,1];[11,2];[10,1];[10,2];[10,3];[13,1];[13,2];[13,3];...
    [5,1];[5,2];[5,3];[14,1];[14,2];[14,3]]; % Used to set the 
                                                           % voltage values in Vi for
                                                           % the new iteration when calculating
                                                           % EV load.
                                                                             
% ViSettingTemp = [[10,1];[10,2];[10,3];[13,1];[13,2];[13,3];...
%     [5,1];[5,2];[5,3];[14,1];[14,2];[14,3];[4,1];[4,2];[4,3]]; 
% % 645, 671, 692, 680, 634, I can make a new Vi later

% Declared once
lossesBus = {'Transformer.SUB'; 'Transformer.XFM1'; 'Line.650632'; 'Line.632671'; 'Line.671680';
            'Line.632633'; 'Line.632645'; 'Line.645646'; 'Line.692675'; 'Line.671684'; 'Line.684611';
            'Line.684652'; 'Line.671692'};

% Defined once
load('pNames.mat');
temptemp = pNames(16:end);
...
% DON'T DELETE - This shows the method.
...
LoadStart = 16;
phaseWiseVec = [4 5 6 0 0 1 2 3 ... % Ind loads
                1 2 3 1 2 1 3 1 2 3 ... % Res Loads
                1 2 3 1 2 3 1 2 1 3 ...
                1 2 3 1 2 3 1 2 3 1 2 3]; % EV Loads
                                    % Is used in the
                                    % fcn getPs to get the 
                                    % phase wise powers from
                                    % exported power values
% phaseWiseVecTemp = [1 2 3 1 2 3 1 2 3 1 2 3];                                            
                                            
numTrafo = 6; % 5 distr. trafo's and one substation trafo

deployedLoadkW = []; % Load INPUT to OpenDSS
deployedLoadkVAR = [];
...
%         evPowerFactor, busNumPh, bikepdf(i));
    ...
    [Pev1stSlot, Qev1stSlot] = getEvLoad(Vi, ev1stSlot, [calcCarPavg(evCarkwh, evCarV, evCarChTime) calcBikePavg(evBikeAh, evBikeV, evBikeChTime, 1)],...
        evPowerFactor, busNumPh, coordMultTimeBlock(i,1)); % Replace 3*ones(1,5) with busNumPh new
    
    [Pev2ndSlot, Qev2ndSlot] = getEvLoad(Vi, ev2ndSlot, [calcCarPavg(evCarkwh, evCarV, evCarChTime) calcBikePavg(evBikeAh, evBikeV, evBikeChTime, 1)],...
        evPowerFactor, busNumPh, coordMultTimeBlock(i,2)); % Replace 3*ones(1,5) with busNumPh new
    
    [Pev3rdSlot, Qev3rdSlot] = getEvLoad(Vi, ev3rdSlot, [calcCarPavg(evCarkwh, evCarV, evCarChTime) calcBikePavg(evBikeAh, evBikeV, evBikeChTime, 1)],...
        evPowerFactor, busNumPh, coordMultTimeBlock(i,3)); % Replace 3*ones(1,5) with busNumPh new
    
    [Pev4thSlot, Qev4thSlot] = getEvLoad(Vi, ev4thSlot, [calcCarPavg(evCarkwh, evCarV, evCarChTime) calcBikePavg(evBikeAh, evBikeV, evBikeChTime, 1)],...
        evPowerFactor, busNumPh, coordMultTimeBlock(i,4)); % Replace 3*ones(1,5) with busNumPh new
    
%     PevTotTemp = PevCarTemp + PevBikeTemp;
%     QevTotTemp = QevCarTemp + QevBikeTemp;
    
    PevTotTemp2 = Pev1stSlot + Pev2ndSlot + Pev3rdSlot + Pev4thSlot;
    QevTotTemp2 = Qev1stSlot + Qev2ndSlot + Qev3rdSlot + Qev4thSlot;
    
    makingLoads([str_part str_part2], [kwLoopInd kwLoopRes PevTotTemp2'],...
        [kvarLoopInd kvarLoopRes QevTotTemp2']); % make the loadss.txt file
    ...
    [tempTrafokw, tempTrafokvar, tempLoadkw, tempLoadkvar]...
         = getPs(xlsread('MasterIEEE13_EXP_P_BYPHASE.CSV',1,'E2:J56'),...
         LoadStart, [phaseWiseVec], numTrafo); 
                                             % Extract the 
                                             % power values for all loads
                                             % and trafos
                                             
    deployedLoadkW = [deployedLoadkW; kwLoopInd kwLoopRes PevTotTemp2'];
    deployedLoadkVAR = [deployedLoadkVAR; kvarLoopInd kvarLoopRes QevTotTemp2'];    
    
    industLdkw = [industLdkw; sum(tempLoadkw(1:industLdNum,:), 1)];
    industLdkvar = [industLdkvar; sum(tempLoadkvar(1:industLdNum,:), 1)];
    
    resLdkw = [resLdkw; sum(tempLoadkw(industLdNum+1:industLdNum+1+resLdNum,:), 1)];
    resLdkvar = [resLdkvar; sum(tempLoadkvar(industLdNum+1:industLdNum+1+resLdNum,:), 1)];
...
%     resLdkvar = [resLdkvar; sum(tempLoadkvar(industLdNum+1:end,:), 1)];
    ...
    % ViTemp = getNewVi(Vpu1hr, ViSettingTemp, 3*5); % I'll update it later
...
save('ev90Pent','V1pu','V2pu','V3pu',...
    ...
    'subkw','subkvar', 'industLdkw','industLdkvar','resLdkw','resLdkvar',...
    'lossesBus','PevTot','QevTot',...
    'resRatedkw','resRatedkvar','industRatedkw','industRatedkvar',...
    'subI','xfm634I','xfm646I','xfm675I','xfm652I','xfm611I','bus680I','bus692I',...
    'PLoss','QLoss');

% save('mults','coordMultTimeBlock','resMult','industMult');

clc;

% close all;
% figure;
% plot(deployedLoadkW(:,1:8));
% legend(pNames(16:16+8-1));
% figure;
% plot(deployedLoadkW(:,9:18));
% legend(pNames(24:33));
% figure;
% plot(deployedLoadkW(:,19:28));
% legend(pNames(34:43));
% figure;
% plot(deployedLoadkW(:,29:end));
% legend(pNames(44:end));
% figure;
% plot(deployedLoadkW(:,9:11)+deployedLoadkW(:,19:21));
% 
% figure;
% plot(resMult);
% hold on; plot(industMult);
...
normalUncoord = load('C:\Users\Abeer\OneDrive - Habib University\V2G Potential FYP\FYP codes & data\Capstone II\03e running the EV loads\normal.mat');
...
%%
% Total load (residential + industrial) graphs AT UNCOORD
figure('Renderer', 'painters', 'Position', [200 -100 1100 700]);
hold on; 
plot(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2),'Color','blue','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
plot(sum(ev90Uncoord.PevTot, 2) + sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2),'Color','Red','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
hold off;

xticks(linspace(1,24*nptMult,7));
xticklabels({'12:00 am','04:00 am','08:00 am','12:00 pm','04:00 pm','08:00 pm','12:00 am'});
title('Daily load profile for all loads at 90% EV penetration uncoord', 'FontSize', titleFontSize); xlabel('Time',...
...
ylim([(floor(min(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*450),...
    (ceil(max(sum(ev90Uncoord.PevTot, 2) + sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*500)])
lgnd = legend(["Load without EVs","Load with EVs"],'Location','southoutside','orientation','horizontal');
...
plot(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2),'Color','blue','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
plot(sum(PevTot, 2) + sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2),'Color','Red','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
hold off;

xticks(linspace(1,24*nptMult,7));
xticklabels({'12:00 am','04:00 am','08:00 am','12:00 pm','04:00 pm','08:00 pm','12:00 am'});
title('Daily load profile for all loads at 90% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
xlim([1 24*nptMult]); grid on;
ylim([(floor(min(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*450),...
    (ceil(max(sum(PevTot, 2) + sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*500)])
lgnd = legend(["Load without EVs","Load with EVs"],'Location','southoutside','orientation','horizontal');
    lgnd.FontSize = lgndFontSize; clear str;

%% Voltages for uncoord (check busNames for voltage values)

for i = 2:length(busNames) % busVSet) % ignoring SOURCEBUS, use j for busVSet
    % i = busVSet(j);
    figure('Renderer', 'painters', 'Position', [200 -100 1100 700])
    hold on;
    ...
    title(strcat("Voltage variation for bus", " ", busNames(i),", ","at 90 percent EV penetration uncoord (base kV ="," ",...
        num2str(basekVs(i))," ","kV_{rms})")); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Voltage in PU', 'FontSize', axisFontSize);
    ax = gca; ax.FontSize = axisFontSize;
    
    xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am','12:00 pm','04:00 pm','08:00 pm','12:00 am'});
    ylim([min([ev90Uncoord.V1pu(:,i)' ev90Uncoord.V2pu(:,i)' ev90Uncoord.V3pu(:,i)' 0.95])-0.01 1.1])
    xlim([1 nptMult*24]);
end
...
for i = 2:length(busNames) % busVSet) % ignoring SOURCEBUS, use j for busVSet
    ...
    title(strcat("Voltage variation for bus", " ", busNames(i),", ","at 90 percent EV penetration (base kV ="," ",...
        num2str(basekVs(i))," ","kV_{rms})")); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Voltage in PU', 'FontSize', axisFontSize);
    ax = gca; ax.FontSize = axisFontSize;
...
%%
% Plotting 24-hr flow at substation AT UNCOORD
figure('Renderer', 'painters', 'Position', [200 -100 1100 700])
plot(ev90Uncoord.subkw(:,1),'linewidth',plotWidth); hold on;
plot(ev90Uncoord.subkw(:,2),'linewidth',plotWidth+0.2); 
plot(ev90Uncoord.subkw(:,3),'linewidth',plotWidth+0.4);
plot(sum(ev90Uncoord.subkw, 2),'linewidth',plotWidth+0.6);
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Substation (bus 650) active power flow at 90% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
...
ylim([floor(min(ev90Uncoord.subkw, [], 'all'))/500 ceil(max( [sum(ev90Uncoord.subkw, 2); ratedSubkw]))*1.25])
xlim([1 24*nptMult]);
...
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
ylim([floor(min(subkw, [], 'all'))/500 ceil(max( [sum(subkw, 2); ratedSubkw]))*1.25])
xlim([1 24*nptMult]);
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (634) power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Sum of" + newline + "all phases";
ylim([floor(min(min(xfmkw634), [], 'all')/500) ceil(max( [sum(xfmkw634, 2); ratedDisTr634kw]))*1.25])
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (652) power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Sum of" + newline + "all phases";
ylim([floor(min((xfmkw652), [], 'all'))/500 ceil(max( [sum(xfmkw652, 2); ratedDisTr652kw]))*1.25])
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (611) power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Sum of" + newline + "all phases";
ylim([floor(min((xfmkw611), [], 'all'))/500 ceil(max( [sum(xfmkw611, 2); ratedDisTr611kw]))*1.25])
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (675) power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Sum of" + newline + "all phases";
ylim([floor(min((xfmkw675), [], 'all'))/500 ceil(max( [sum(xfmkw675, 2); ratedDisTr675kw]))*1.25])
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (646) power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Sum of" + newline + "all phases";
ylim([floor(min((xfmkw646), [], 'all'))/500 ceil(max( [sum(xfmkw646, 2); ratedDisTr646kw]))*1.25])
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str,'Rated kW'},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;

%% Plotting 24-hr current flow at 2ndary terminal for sub AT UNCOORD
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev90Uncoord.subI(:,1),'linewidth',plotWidth); hold on;
plot(ev90Uncoord.subI(:,2),'linewidth',plotWidth*1.12); 
plot(ev90Uncoord.subI(:,3),'linewidth',plotWidth*1.14); 
% plot(sum(xfmkw646, 2),'linewidth',plotWidth*1.16);
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Substation (Sub) current flow at 40% EV penetration uncoord', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
ylim([floor(min((ev90Uncoord.subI), [], 'all'))/500 ceil(max([max(ev90Uncoord.subI,[] ,'all'); 1.1*5500/sqrt(3)/11]))*1.15]) % Max rating are from max current,
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Substation (Sub) current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (634) current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
%%
% Plotting 24-hr current flow at 2ndary terminal for 634 AT UNCOORD
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev90Uncoord.xfm634I(:,1),'linewidth',plotWidth); hold on;
plot(ev90Uncoord.xfm634I(:,2),'linewidth',plotWidth*1.12); 
plot(ev90Uncoord.xfm634I(:,3),'linewidth',plotWidth*1.14); 
% plot(sum(xfmkw646, 2),'linewidth',plotWidth*1.16);
plot(ones(1,npts)*1000*1.1/0.4/sqrt(3),'linewidth',plotWidth);
% plot(ones(1,npts)*(1000*1.5/0.4/sqrt(3)),'linewidth',plotWidth);
hold off;
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (634) current flow at 90% EV penetration at uncoord', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
ylim([floor(min((ev90Uncoord.xfm634I), [], 'all'))/500 ceil(max([max(ev90Uncoord.xfm634I,[] ,'all'); 1000*1.1/0.4/sqrt(3)]))*1.15]) % Max rating are from max current,
                                                                                                      % normamps 
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
%%
% Plotting 24-hr current flow at 2ndary terminal for 675 AT UNCOORD
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev90Uncoord.xfm675I(:,1),'linewidth',plotWidth); hold on;
plot(ev90Uncoord.xfm675I(:,2),'linewidth',plotWidth*1.12); 
plot(ev90Uncoord.xfm675I(:,3),'linewidth',plotWidth*1.14); 
% plot(sum(xfmkw646, 2),'linewidth',plotWidth*1.16);
plot(ones(1,npts)*1000*1.1/0.4/sqrt(3),'linewidth',plotWidth);
% plot(ones(1,npts)*(1000*1.5/0.4/sqrt(3)),'linewidth',plotWidth);
hold off;
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (675) current flow at 40% EV penetration uncoord', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
ylim([floor(min((ev90Uncoord.xfm675I), [], 'all'))/500 ceil(max([max(ev90Uncoord.xfm675I,[] ,'all'); 1000*1.1/0.4/sqrt(3)]))*1.15]) % Max rating are from max current,
                                                                                                      % normamps 
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (675) current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Bus 680 current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
%% 
% Plotting 24-hr current flow at 2ndary terminal for bus 692 AT UNCOORD
% IMPORTANT! I'll mention the current rating separately
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev90Uncoord.bus692I(:,1),'linewidth',plotWidth); hold on;
plot(ev90Uncoord.bus692I(:,2),'linewidth',plotWidth*1.12); 
plot(ev90Uncoord.bus692I(:,3),'linewidth',plotWidth*1.14); 
% plot(ones(1,npts)*726,'linewidth',plotWidth); % CURRENT RATING!!!
% plot(ones(1,npts)*(1000*1.5/0.4/sqrt(3)),'linewidth',plotWidth);
hold off;
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Bus 692 current flow at 90% EV penetration at uncoord', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
ylim([floor(min((ev90Uncoord.bus692I), [], 'all'))/500 ceil(max([max(ev90Uncoord.bus692I,[] ,'all'); 0]))*1.15]) % Max rating are from max current,
                                                                                                      % normamps 
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Bus 692 current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Distribution transformer (652) current flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Current (A)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
...
%% Plotting losses for UNCOORD 

losses = [sum(ev90Uncoord.PLoss, 1); sum(PLoss, 1)];

figure;
plot(losses','linewidth',plotWidth);
...
title('Total daily losses of the system', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
% str = "Sum of" + newline + "all phases";
lgnd = legend({'40% EV penetration with uncoord',...
    '40% EV penetration with coord'},'Location',...
