...
offsetb = 0.2; % ADDS to industMult values
...
% industMult = skewedgaussian(linspace(-0.5,1.5,npts),1.25,0,10) + noiseStd.*randn(1,npts);
% industMult = industMult./max(industMult)/multb;
% industMult = smooth(abs(industMult./max(industMult) - offsetb),4)';
% industMult = circshift(industMult, 5);
...
ev1stSlot = readmatrix('data/1stSlotEV40.csv');
ev2ndSlot = readmatrix('data/2ndSlotEV40.csv');
ev3rdSlot = readmatrix('data/3rdSlotEV40.csv');
ev4thSlot = readmatrix('data/4thSlotEV40.csv');
...
%%
% Total load (residential + industrial + ev) graphs AT UNCOORD
...
plot(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2),'Color','blue','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
plot(sum(ev40Uncoord.PevTot, 2) + sum(ev40Uncoord.resLdkw, 2) + sum(ev40Uncoord.industLdkw, 2),'Color','Red','Marker','o','MarkerFaceColor','white',...
    'linewidth',plotWidth);
hold off;

xticks(linspace(1,24*nptMult,7));
xticklabels({'12:00 am','04:00 am','08:00 am','12:00 pm','04:00 pm','08:00 pm','12:00 am'});
title('Daily load profile for all loads at 40% EV penetration uncoord', 'FontSize', titleFontSize); xlabel('Time',...
...
ylim([(floor(min(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*450),...
    (ceil(max(sum(ev40Uncoord.PevTot, 2) + sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*500)])
...
xticks(linspace(1,24*nptMult,7));
xticklabels({'12:00 am','04:00 am','08:00 am','12:00 pm','04:00 pm','08:00 pm','12:00 am'});
title('Daily load profile for all loads at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
xlim([1 24*nptMult]); grid on;
ylim([(floor(min(sum(normalUncoord.resLdkw, 2) + sum(normalUncoord.industLdkw, 2))/500)*450),...
...
for i = 2:length(busNames) % busVSet) % ignoring SOURCEBUS, use j for busVSet
    ...
    title(strcat("Voltage variation for bus", " ", busNames(i),", ","at 40 percent EV penetration uncoord (base kV ="," ",...
...
for i = 2:length(busNames) % busVSet) % ignoring SOURCEBUS, use j for busVSet
    ...
    title(strcat("Voltage variation for bus", " ", busNames(i),", ","at 40 percent EV penetration (base kV ="," ",...
        num2str(basekVs(i))," ","kV_{rms})")); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Voltage in PU', 'FontSize', axisFontSize);
    ax = gca; ax.FontSize = axisFontSize;
...
figure('Renderer', 'painters', 'Position', [200 -100 1100 700])
plot(ev40Uncoord.subkw(:,1),'linewidth',plotWidth); hold on;
plot(ev40Uncoord.subkw(:,2),'linewidth',plotWidth+0.2); 
plot(ev40Uncoord.subkw(:,3),'linewidth',plotWidth+0.4);
plot(sum(ev40Uncoord.subkw, 2),'linewidth',plotWidth+0.6);
...
xticks(linspace(1,24*nptMult,7));
    xticklabels({'12:00 am','04:00 am','08:00 am',...
        '12:00 pm','04:00 pm','08:00 pm','12:00 am'});
grid on;
title('Substation (bus 650) active power flow at 40% EV penetration', 'FontSize', titleFontSize); xlabel('Time',...
    'FontSize', axisFontSize); ylabel('Varying power (kW)', 'FontSize', axisFontSize);
ax = gca; ax.FontSize = axisFontSize;
...
ylim([floor(min(ev40Uncoord.subkw, [], 'all'))/500 ceil(max( [sum(ev40Uncoord.subkw, 2); ratedSubkw]))*1.25])
xlim([1 24*nptMult]);
...
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev40Uncoord.subI(:,1),'linewidth',plotWidth); hold on;
plot(ev40Uncoord.subI(:,2),'linewidth',plotWidth*1.12); 
plot(ev40Uncoord.subI(:,3),'linewidth',plotWidth*1.14); 
% plot(sum(xfmkw646, 2),'linewidth',plotWidth*1.16);
...
ylim([floor(min((ev40Uncoord.subI), [], 'all'))/500 ceil(max([max(ev40Uncoord.subI,[] ,'all'); 1.1*5500/sqrt(3)/11]))*1.15]) % Max rating are from max current,
...
figure('Renderer', 'painters', 'Position', [200 100 1100 700])
plot(ev40Uncoord.xfm675I(:,1),'linewidth',plotWidth); hold on;
plot(ev40Uncoord.xfm675I(:,2),'linewidth',plotWidth*1.12); 
plot(ev40Uncoord.xfm675I(:,3),'linewidth',plotWidth*1.14); 
% plot(sum(xfmkw646, 2),'linewidth',plotWidth*1.16);
plot(ones(1,npts)*1000*1.1/0.4/sqrt(3),'linewidth',plotWidth);
% plot(ones(1,npts)*(1000*1.5/0.4/sqrt(3)),'linewidth',plotWidth);
hold off;
...
ax = gca; ax.FontSize = axisFontSize;
str = "Normal Ampere rating";
ylim([floor(min((ev40Uncoord.xfm675I), [], 'all'))/500 ceil(max([max(ev40Uncoord.xfm675I,[] ,'all'); 1000*1.1/0.4/sqrt(3)]))*1.15]) % Max rating are from max current,
                                                                                                      % normamps 
xlim([1 24*nptMult]);
lgnd = legend({'Phase a','Phase b','Phase c',str},'Location','southoutside','orientation','horizontal');
lgnd.FontSize = lgndFontSize; clearvars str;
...
losses = [sum(ev40Uncoord.PLoss, 1); sum(PLoss, 1)];
